...
const state = {
  ...
  solo:null, pendingChallenge:null, pendingSoloCard:null, pendingCard:null, unoTimer:null, muted:false, platform:SAVED_PLATFORM, currentMapTheme:'saloon', actionTimers:new Map(), typingTimer:null, musicTimer:null
};
...
function applySoloCard(card,chosenColor){const g=state.solo;const i=g.player.findIndex(x=>x.id===card.id);if(i<0)return;g.player.splice(i,1);g.pile.push(g.discard);g.discard=card;g.color=card.color==='black'?(COLORS.includes(chosenColor)?chosenColor:COLORS[Math.floor(Math.random()*4)]):card.color;Sound.card();if(card.type==='draw2')drawSolo(g.bot,2);if(card.type==='draw4')drawSolo(g.bot,4);if(g.player.length===0)return finishSolo(true);if(g.player.length===1){g.unoDeadline=Date.now()+3200;}if(card.type==='skip'||card.type==='reverse'){g.turn='bot';renderSolo();setTimeout(botTurn,3000+Math.floor(Math.random()*7001));return;}g.turn='bot';renderSolo();setTimeout(botTurn,3000+Math.floor(Math.random()*7001));}
...
function callUno(){if(state.solo){if(state.solo.player.length!==1)return toast('Você só chama UNO com 1 carta.','error');state.solo.unoDeadline=null;clearUnoTimer();Sound.ok();toast('📣 UNOOOOOO!','success',2200);renderSolo();return;}if(state.currentRoom?.started){state.socket?.emit('game:uno');}}
