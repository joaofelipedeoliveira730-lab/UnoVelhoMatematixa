...
function renderOnlineGame(game){
  ...
  const hand=$('#playerHand');if(hand){const cards=Array.isArray(game.hand)?game.hand:[];hand.innerHTML=cards.length?cards.map((c,i)=>cardHtml(c,i,cards.length)).join(''):'<div class="hand-empty">AGUARDE SUAS CARTAS...</div>'; }
  const ops=$('#opponents');
...
function handleGameAction(action){if(!action)return;if(action.type==='play')animateCardMove(action);if(action.type==='draw')animateDraw(action);if(action.type==='uno-penalty'){Sound.bad();toast(`⚠️ ${action.username} esqueceu o UNO e comprou ${action.count} cartas!`,'error',2600);}}
...
function clearUnoTimer(){if(state.unoTimer){clearInterval(state.unoTimer);state.unoTimer=null;}state._unoDeadline=null;}
function updateUnoButton(active,deadline){const b=$('#btnUno');if(!b)return;if(!active){b.classList.add('hidden');b.textContent='UNO!';clearUnoTimer();return;}b.classList.remove('hidden');state._unoDeadline=deadline||Date.now()+3000;b.textContent='UNO!';if(state.unoTimer)clearInterval(state.unoTimer);state.unoTimer=setInterval(()=>{const left=Math.max(0,state._unoDeadline-Date.now());b.style.setProperty('--uno-progress',`${left/3000}`);b.textContent=left>0?`UNO! ${Math.ceil(left/1000)}`:'UNO!';if(left<=0){clearUnoTimer();b.classList.add('expired');setTimeout(()=>b.classList.add('hidden'),350);}},80);}
function callUno(){if(state.solo){if(state.solo.player.length!==1)return toast('Você só chama UNO com 1 carta.','error');state.solo.unoDeadline=null;clearUnoTimer();Sound.ok();toast('📣 UNOOOOOO!','success',2200);renderSolo();return;}if(state.currentRoom?.started){Sound.uno();state.socket?.emit('game:uno');}}
function exitGame(){clearUnoTimer();state.solo=null;state._onlineGame=null;state.pendingCard=null;state.pendingSoloCard=null;hide('#colorModal');navigate(state.currentRoom?'room':'lobby');}
...
// ---------------- PASSE DE NÍVEL ----------------
function passRewardIcon(r){if(r.title==='pass_title_lenda')return '👑';if(r.itemId?.includes('hat'))return '🧢';if(r.itemId?.startsWith('title'))return '🏷️';return '🪙';}
async function openBattlePass(){navigate('battlePass');try{state.passData=await get('/pass');renderBattlePass();}catch(e){toast(e.message,'error');}}
function renderBattlePass(){const d=state.passData;if(!d)return;const level=Math.max(1,Math.min(100,Number(d.level)||1));$('#passCurrentLevel').textContent=level;$('#passXpText').textContent=`${fmt(d.xp)} XP`;const next=level<100?Math.floor(100*Math.pow(level,1.45)):d.xp;$('#passNextText').textContent=level>=100?'NÍVEL MÁXIMO':`Próximo: ${fmt(next)} XP`;const base=level<=1?0:Math.floor(100*Math.pow(level-1,1.45));const pct=level>=100?100:Math.max(0,Math.min(100,((d.xp-base)/Math.max(1,next-base))*100));$('#passXpBar').style.width=pct+'%';const claimed=new Set(d.claimed||[]);$('#passGrid').innerHTML=d.levels.map(r=>{const unlocked=r.level<=level,done=claimed.has(r.level);return `<article class="pass-card ${unlocked?'unlocked':'locked'} ${done?'claimed':''}"><div class="pass-level">${r.level}</div><div class="pass-reward-icon">${passRewardIcon(r)}</div><b>${r.coins} 🪙</b>${r.itemId?`<small>${escapeHtml(itemName(r.itemId))}</small>`:''}${r.title?`<small>${escapeHtml(itemName(r.title))}</small>`:''}<button class="btn ${done?'btn-owned':'btn-primary'} pass-claim" data-pass-level="${r.level}" ${!unlocked||done?'disabled':''}>${done?'✓ COLETADO':unlocked?'COLETAR':'🔒 '+r.level}</button></article>`}).join('');}
async function claimPass(all=false,level=null){if(!state.passData)return;const levels=all?(state.passData.levels||[]).filter(r=>r.level<=state.passData.level&&!state.passData.claimed.includes(r.level)).map(r=>r.level):[Number(level)];if(!levels.length)return toast('Não há recompensas novas para coletar.','info');try{const d=await post('/pass/claim',{levels});state.passData.claimed=[...(state.passData.claimed||[]),...(d.claimed||[])];if(d.user)state.user=d.user;state.inventory=(await get('/inventory')).items||[];updateUserUI();renderBattlePass();toast(d.message,'success');Sound.ok();}catch(e){toast(e.message,'error');Sound.bad();}}
