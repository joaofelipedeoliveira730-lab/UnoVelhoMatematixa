...
function startComingSoonMode(room){room.started=false;room.locked=false;room.game={mode:room.options.gameMode,waiting:true,createdAt:Date.now()};io.to(`room:${room.code}`).emit('toast',{type:'info',message:`🎮 Sala de ${String(room.options.gameMode).toUpperCase()} criada. A mesa está pronta para receber o modo de jogo.`});emitRoom(room);}
